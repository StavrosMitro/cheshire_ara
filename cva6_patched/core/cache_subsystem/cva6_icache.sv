...
#(
  ...
  ///////////////////////////////////////////////////////
  logic addr_ni;
  logic addr_ni_dram, addr_ni_periph;
  logic [63:0] fetch_paddr64;
  // K5 3b: split into DRAM (index 0) / peripheral (index 1) rules stays --
  // not for a runtime switch anymore (ni_disable_i removed; rule 0 is now
  // permanently dead on zcu102, see cheshire_pkg.sv Cva6NiDramRule), but
  // as a permanent watchdog on this exact deadlock: with rule 0 dead,
  // addr_ni_dram should only ever move on peripheral fetches from here on
  // (probe it directly if this ever needs checking). addr_ni's meaning and
  // the fetch gate below (the READ-state spec/addr_ni check) are unchanged
  // -- only what feeds addr_ni changes.
  assign fetch_paddr64 = {{64 - CVA6Cfg.PLEN{1'b0}}, areq_i.fetch_paddr};
  assign addr_ni_dram = config_pkg::range_check(
      CVA6Cfg.NonIdempotentAddrBase[0], CVA6Cfg.NonIdempotentLength[0], fetch_paddr64);
  assign addr_ni_periph = config_pkg::range_check(
      CVA6Cfg.NonIdempotentAddrBase[1], CVA6Cfg.NonIdempotentLength[1], fetch_paddr64);
  assign addr_ni = addr_ni_dram || addr_ni_periph;
