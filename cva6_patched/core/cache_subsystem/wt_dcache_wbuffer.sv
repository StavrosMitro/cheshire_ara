...
#(
  ...
  logic is_nc_miss;
  logic is_ni;
  logic is_ni_dram, is_ni_periph;
  logic [63:0] req_paddr64;
  ...
  // Non-idempotent if request goes to NI region. K5 3b: split into the DRAM
  // (index 0) and peripheral (index 1) rules stays as a permanent watchdog
  // -- rule 0 is now permanently dead on zcu102 (Cva6NiDramRule=0), so
  // is_ni_dram should only ever read 0 from here on.
  assign req_paddr64 = {
    {64 - CVA6Cfg.DCACHE_TAG_WIDTH - CVA6Cfg.DCACHE_INDEX_WIDTH{1'b0}},
    req_port_i.address_tag,
    {CVA6Cfg.DCACHE_INDEX_WIDTH{1'b0}}
  };
  assign is_ni_dram = config_pkg::range_check(
      CVA6Cfg.NonIdempotentAddrBase[0], CVA6Cfg.NonIdempotentLength[0], req_paddr64);
  assign is_ni_periph = config_pkg::range_check(
      CVA6Cfg.NonIdempotentAddrBase[1], CVA6Cfg.NonIdempotentLength[1], req_paddr64);
  assign is_ni = is_ni_dram || is_ni_periph;
